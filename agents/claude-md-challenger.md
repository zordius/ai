...
[POSITIVE only] Executable path: (1) Confirm the external signal ({task context / platform enforcement}) is sufficient at runtime — an agent encountering the situation would reach the correct behavior without this section present. (2) Search the config tree for references to this section's trigger-word or heading — `grep -rn` the section title and any trigger phrases across `.claude/` (or equivalent config root); if any other doc links or points to this section, resolve those references before deleting (update or remove the pointer). Only proceed to (3) if no live references remain. (3) Delete the section from CLAUDE.md; no stub pointer needed. (4) If Q10 was the deciding factor, consider archiving the platform-fact to a reference doc for audit purposes — it is correct but not load-bearing. Caller must verify (1) before executing (3).
